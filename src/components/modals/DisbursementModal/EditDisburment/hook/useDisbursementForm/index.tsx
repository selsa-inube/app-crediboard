...
export const useDisbursementForm = (props: IUseDisbursementFormProps) => {
    ...
    const [currentIdentification, setCurrentIdentification] = useState(identificationNumber);
    ...
    const isInvalidAmount = isOverLimit || (Boolean(formik.touched[optionNameForm]?.amount) && Boolean(formik.errors[optionNameForm]?.amount));
    ...
    }, [currentAmountValue, currentCheckValue, getTotalAmount, initialValues.amount, optionNameForm, formik]);

    const restoreCustomerDataFields = useCallback(() => {
        const person = customerData?.generalAttributeClientNaturalPersons?.[0];

        formik.setFieldValue(`${optionNameForm}.documentType`, person?.typeIdentification || "");
        formik.setFieldValue(`${optionNameForm}.identification`, customerData?.publicCode || "");
        ...
        formik.setFieldValue(`${optionNameForm}.birthdate`, person?.dateBirth || "");
        formik.setFieldValue(`${optionNameForm}.phone`, person?.cellPhoneContact || "");
        formik.setFieldValue(`${optionNameForm}.mail`, person?.emailContact || "");
        formik.setFieldValue(`${optionNameForm}.city`, person?.zone?.split("-")[1] || "");
    ...
    const clearFields = useCallback(() => {
        const fields = [
            "documentType", "name", "lastName", "sex", "birthdate",
            "phone", "mail", "city", "identification", "accountNumber",
        ];
    ...
    useEffect(() => {
        ...
        const dataKey = Object.keys(initialValues).find(key => key === optionNameForm) as keyof IDisbursementGeneral;
        const initialData = initialValues[dataKey];

        if (initialData && typeof initialData === 'object' && 'amount' in initialData) {
            const currentToggle = formik.values[optionNameForm]?.toggle;

            if (optionNameForm === "Cash" && Number(initialValues.amount) <= 0 && customerData) {
    ...
    }, [initialValues, optionNameForm, customerData, formik, restoreCustomerDataFields]);
    ...
    }, [identificationValue, currentIdentification, isAutoCompleted, formik, optionNameForm, nameValue]);

    useEffect(() => {
        const identification = formik.values[optionNameForm]?.identification;
        const borrowerIdentification = prospectData?.borrowers?.[0]?.borrowerIdentificationNumber;
        ...
        const fetchCustomer = async () => {
            try {
                ...
                if (customer?.publicCode && data && customer.publicCode !== customerData?.publicCode) {
                    setCurrentIdentification(identification);

                    formik.setFieldValue(`${optionNameForm}.documentType`, data.typeIdentification || "");
                    formik.setFieldValue(`${optionNameForm}.name`, data.firstNames || "");
                    formik.setFieldValue(`${optionNameForm}.lastName`, data.lastNames || "");
                    formik.setFieldValue(`${optionNameForm}.sex`, data.gender || "");
                    formik.setFieldValue(`${optionNameForm}.birthdate`, data.dateBirth || "");
                    formik.setFieldValue(`${optionNameForm}.phone`, data.cellPhoneContact || "");
                    formik.setFieldValue(`${optionNameForm}.mail`, data.emailContact || "");
                    formik.setFieldValue(`${optionNameForm}.city`, data.zone?.split("-")[1] || "");
