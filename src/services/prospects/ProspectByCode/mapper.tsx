import { IProspect, IBorrower, ICreditProduct, IBorrowerProperty, IOutlay, IOrdinaryInstallmentsForPrincipal } from "./types";

export const mapperProspectResponseToIProspect = (response: IProspect): IProspect => {
  const prospect: IProspect = {
    ...
    preferredPaymentChannelAbbreviatedName: response.preferredPaymentChannelAbbreviatedName,
    ...
    borrowers: response.borrowers.map((borrower: IBorrower): IBorrower => ({
      ...
      borrowerProperties: borrower.borrowerProperties.map((property: IBorrowerProperty): IBorrowerProperty => ({
    ...
    creditProducts: response.creditProducts.map((product: ICreditProduct): ICreditProduct => ({
    ...
    outlays: response.outlays.map((outlay: IOutlay): IOutlay => ({
      date: outlay.date,
